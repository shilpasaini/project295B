...
app.get("/chart",function(req,res){
	...
	mongo.getSplineChartData(function(err,reslt){
		if(err){
				...
				if(err){
					...
					for(var k=result.length-1;k>=0;k--){
						...
						var objTemp=[];
		     			...
		     			objTemp[2] = result[k].objTempMin;
		     			objTemp[3] = result[k].ambTempMax;
		     			objTemp[4] = result[k].ambTempMin
		     			maxObj.push(objTemp[0],objTemp[1]);
		     			minObj.push(objTemp[0],objTemp[2]);
						}
			...
			res.render('charts',{"minObj":JSON.stringify(minObj),"maxObj":JSON.stringify(maxObj),"dates":JSON.stringify(dates),"ambTemp":JSON.stringify(temp)});
		}
	});
